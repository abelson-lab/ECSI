...
function(cosmic_mutations_path = "hemCOSMIC.csv") {
  ...
  cosmic_mutations_df <- fread(cosmic_mutations_path) 
  
  # convert to VRanges
  cosmic_mutations <- with(cosmic_mutations_df, VRanges( 
    seqnames = paste0("chr",Chr), ranges = IRanges(Start, End), ref = Ref, alt = Alt))
  
  # load in metadata
  mcols(cosmic_mutations) <- dplyr::select(cosmic_mutations_df, -c("Chr", "Start", "End", "Ref", "Alt"))
