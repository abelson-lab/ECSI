...
Note - If you wanted to generate variant calls only for specific mutations (i.e. specified ref + alt) rather than the position alone, you can use the load_recurrent_mutatations function to load your specific mutations from a tab-delimited file with four columns: “chr, pos, ref, alt”. The intersect_VRanges function will recognize the format and adapt accordingly.
...
# use sample to generate the error models
	samp_models <- samp %>%
		filter_model_input(., flagged_alleles, MAF_cutoff = 0.001, VAF_cutoff = 0.05, 
		                   MAPQ_cutoff = 59, recurrent_mutations = hemeCOSMIC_10) %>%
		generate_all_models()
