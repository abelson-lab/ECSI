...
variant_calls_unlisted <- variant_calls %>% unlist() %>% correct_pvalues(., by_sample = FALSE)
...
To start, we will load the following annotation packages specific to hg19:

```{r}
library(BSgenome.Hsapiens.UCSC.hg19)
Hsapiens_hg19 <- BSgenome.Hsapiens.UCSC.hg19
...
Next we will get the gene ids from TxDb and convert them to gene symbols with biomaRt (internet connection required)

```{r}
# Get gene ids
gene_ids <- subsetByOverlaps(genes(txdb_hg19), significant_calls)
gene_ids$gene_name <- biomaRt::select(mart, keys = gene_ids$gene_id, columns = "hgnc_symbol", keytype= "entrezgene")

# Add gene names to metadata in significant_calls
significant_calls$GENE <- gene_ids[subjectHits(findOverlaps(significant_calls, gene_ids))]$gene_name
```

Last we will predict protein coding consequences using predictCoding from the VariantAnnotation package, and annotate our variant calling output with it.

```{r}
# Get amino acid changes
pred <- predictCoding(significant_calls, txdb_hg19, Hsapiens_hg19, ignore.strand = TRUE)
pred <- unique(pred)

# Add annotation to metadata in significant_calls
significant_calls$PROTEINLOC <- as.integer(pred[queryHits(findOverlaps(pred, significant_calls))]$PROTEINLOC)
significant_calls$CONSEQUENCE <- pred[queryHits(findOverlaps(pred, significant_calls))]$CONSEQUENCE
significant_calls$REFCODON <- pred[queryHits(findOverlaps(pred, significant_calls))]$REFCODON
significant_calls$VARCODON <- pred[queryHits(findOverlaps(pred, significant_calls))]$VARCODON
significant_calls$REFAA <- pred[queryHits(findOverlaps(pred, significant_calls))]$REFAA
significant_calls$VARAA <- pred[queryHits(findOverlaps(pred, significant_calls))]$VARAA
```

For easier viewing, we can display this as a tibble object

```{r}
significant_calls %>% tibble::as_tibble()
```
